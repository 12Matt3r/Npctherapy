...
# Character mapping based on provided mapping
character_mapping = {
    # Already properly named
    "Mira the Tired Healer (Fantasy Cleric) office.png": "Mira the Tired Healer (Fantasy Cleric) office.png",
...
def rename_files():
    ...
    if os.path.exists(therapy_dir):
        ...
        for old_name in os.listdir(therapy_dir):
            if old_name.endswith('.png'):
                ...
                if old_name in character_mapping:
                    new_name = character_mapping[old_name]
                    # For therapy office, we need to add " office.png" if not already present
                    if " office.png" not in new_name:
                        new_name = new_name + " office.png"
    ...
    # Handle character habitats files  
    ...
    if os.path.exists(habitats_dir):
        ...
        for old_name in os.listdir(habitats_dir):
            if old_name.endswith('.png'):
                ...
                if old_name in character_mapping:
                    new_name = character_mapping[old_name]
                    # For habitats, we need to add " from [setting].png" if not already present
                    if " from " not in new_name:
                        # Extract character name and add " from [Game Setting].png"
                        new_name = new_name + " from Gaming Environment.png"
